...
./vpn servers
```
If that returns server names, gluetun's API is healthy. Note the orchestrator no longer fetches this list at runtime — it pre-filters the AirVPN queue against the **hardcoded `GLUETUN_ACCEPTED_AIRVPN_SERVERS` list in [orchestrator/config.js](orchestrator/config.js)**. So if `./vpn test` keeps skipping every server, the cause is gluetun's bundled config itself (e.g. a gluetun upgrade changed the accepted names); regenerate the hardcoded list per the TODO in config.js. `./vpn servers` is the live, ad-hoc way to see what the running gluetun actually accepts.

**`./vpn servers` errors with `container not found` or `not running`.**
`./vpn servers` queries gluetun live via `docker exec ... wget http://localhost:8000/v1/servers/airvpn` (gluetun's auth middleware doesn't whitelist that route for cross-container HTTP, hence the in-container `docker exec`). If gluetun-speedtest is exited or missing, `./vpn check` will show it, and the fix is to recreate the base stack with `./vpn deploy`. This command is independent of the orchestrator's hardcoded pre-filter list.
